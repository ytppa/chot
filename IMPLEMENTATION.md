...
- Shadow DOM используется в компонентах интерфейса.
...
- Компоненты используют Shadow DOM.
- События из компонентов отправляются с `bubbles: true` и `composed: true`, если должны доходить до shell.
- Иконки подключены локально через Font Awesome Free SVG subset в `apps/web/public/vendor/fontawesome`; компоненты используют helper `apps/web/src/utils/fontawesome.ts`, который рендерит декоративные mask-иконки без CDN.
- Общие button interactions вынесены в `apps/web/src/utils/button-interactions.ts` и дублируются в глобальных tokens для light DOM: hover слегка затемняет кнопку через `filter: brightness(0.96)`, а `:focus-visible` показывает outline.
...
- `WebSocketClient` умеет подключаться к `/ws`, отправлять `ping`, отправлять `message.send`, переподключаться после обрыва и держать bounded queue событий.
- Для `message.send` клиент хранит неподтвержденные события до `message.ack` и повторно отправляет их после reconnect; `clientNonce` на сервере защищает от дублей.
...
- При открытии/создании чата и после отправки сообщения `x-app-shell` возвращает фокус в textarea composer внутри Shadow DOM.
...
- Все кнопки внутри Shadow DOM компонентов получают `cursor: pointer`, а disabled-кнопки возвращают обычный cursor.
...
- После восстановления WebSocket `x-app-shell` перезагружает direct chats и историю активного чата, чтобы догнать сообщения, пропущенные во время разрыва.
...
- Chat list activity label shows `HH:MM` for today's latest message, `Вчера` for yesterday, `дд мес.` for the current year and `дд.мм.гг` for older years.
...
- Frontend WebSocket reconnect реализован; ручной browser check после перезапуска backend остается полезным перед полировкой MVP.
